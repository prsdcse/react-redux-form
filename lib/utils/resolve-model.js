...
function resolveModel(model, parentModel) {
  if (parentModel) {
    if (model[0] === '.' || model[0] === '[') {
      return "".concat(parentModel).concat(model);
    }
...
function wrapWithModelResolver(WrappedComponent) {
  ...
  function (_PureComponent) {
    ...
    function ResolvedModelWrapper() {
      ...
      return _possibleConstructorReturn(this, _getPrototypeOf(ResolvedModelWrapper).apply(this, arguments));
    }
    ...
    _createClass(ResolvedModelWrapper, [{
      ...
      value: function render() {
        var _this$context = this.context,
            parentModel = _this$context.model,
            localStore = _this$context.localStore;
  ...
  ResolvedModelWrapper.displayName = "Modeled(".concat(WrappedComponent.displayName, ")");
  ResolvedModelWrapper.propTypes = process.env.NODE_ENV !== "production" ? {
